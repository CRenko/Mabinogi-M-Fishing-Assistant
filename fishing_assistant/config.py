...
def _config_from_raw(raw: dict) -> AppConfig:
    ...
    if int(raw.get("schema_version", 0)) < 17:
    ...
    try:
        ...
        raw["recovery_attempt_limit"] = 5
    try:
        raw["recovery_forward_compensation_interval"] = max(
            0,
            min(
                20,
                int(raw.get("recovery_forward_compensation_interval", 2)),
            ),
        )
    except (TypeError, ValueError, OverflowError):
        raw["recovery_forward_compensation_interval"] = 2
    try:
        raw["recovery_forward_compensation_taps"] = max(
            1,
            min(10, int(raw.get("recovery_forward_compensation_taps", 1))),
        )
    except (TypeError, ValueError, OverflowError):
        raw["recovery_forward_compensation_taps"] = 1

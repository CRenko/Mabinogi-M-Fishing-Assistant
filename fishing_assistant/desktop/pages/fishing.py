...
from fishing_assistant.desktop.widgets import Card, DetailsPanel, ScrollSafeComboBox
from fishing_assistant.desktop.pages.sectioned import SectionedPage


class FishingPageMixin:
    """钓鱼设置页面与背包清理选项布局；由 MainWindow 组装，不单独实例化。"""

    def _build_detection_page(self) -> SectionedPage:
        scroll = SectionedPage()
        strategy_section = scroll.add_section("strategy", "收鱼策略")
        recovery_section = scroll.add_section("recovery", "自动续钓")
        cleanup_section = scroll.add_section("cleanup", "背包清理")

        strategy_card = Card()
        strategy_layout = QVBoxLayout(strategy_card)
        configure_card_layout(strategy_layout)
        ...
        mode_two_grid.addWidget(
            self._form_label("最迟收杆时间", "默认 10.5 秒，在普通鱼消失前留出余量。"),
            0,
            1,
        )
        ...
        strategy_section.addWidget(strategy_card)
        strategy_section.addStretch(1)

        recovery_card = Card()
        recovery_layout = QVBoxLayout(recovery_card)
        configure_card_layout(recovery_layout)
        ...
        self.auto_resume_check = QCheckBox("收鱼后自动续钓")
        ...
        self.recovery_mode_combo.addItem("W → S 往返（推荐）", "ws")
        ...
        camera_warning = QLabel("使用手动镜头，镜头对准河岸与角色背部。")
        camera_warning.setObjectName("safetyWarning")
        camera_warning.setWordWrap(True)
        w_only_panel.layout().addWidget(camera_warning)
        ...
        recovery_section.addWidget(recovery_card)

        cleanup_card, cleanup_safety_card = self._build_inventory_cleanup_cards()
        cleanup_section.addWidget(cleanup_card)
        cleanup_section.addWidget(cleanup_safety_card)
    ...
    def _build_inventory_cleanup_cards(self) -> tuple[Card, Card]:
        ...
        configure_card_layout(safety_layout)
        safety_layout.setSpacing(9)
        safety_layout.addWidget(DetailsPanel("执行前安全检查", (
